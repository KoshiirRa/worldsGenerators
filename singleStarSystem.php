...
if ($numPlanets > 0) {
	...
	foreach($planets as $zone) {
		...
		if ($atmosphere[0]!=1 && $size[0] <= 7) {
			...
			echo "<li>".$atmosphereComposition[1]."</li>";
		}
		...
		if ($size[0] <= 7) {
			...
			echo "<li>".$planetHydrosphere[1]."</li>";
		}
		...
		if ($size[0] <= 7 && $atmosphere[0] != 1 && $zone == 2) {
			...
			echo "<li>".$planetClimate."</li>";
		} elseif ($zone == 1) {
			$planetClimate[0] = "Hot";
			echo "<li>Hot Climate</li>";
		} elseif ($zone == 3) {
			$planetClimate[0] = "Cold";
			echo "<li>Cold Climate</li>";
		} else {
			echo "<li>Vacuum Climate (broiling in sunlight, freezing in shadow)</li>";
		}
		...
		if ($size[0] <= 7) {
			...
			echo "<li>".$planetBiosphere[2];
			if ($planetBiosphere[0]!=1) {
				echo "<ul><li>".$planetBiosphere[3];
...
//$size[0] key: 1 = diminutive, 2 = fine, 3 = tiny, 4 = small, 5 = medium, 6 = large, 7 = huge, 8 = small gas giant, 9 = medium gas giant, 10 = huge gas giant, 11 = gargantuan gas giant, 12 = y-class brown dwarf
		...
		if ($numMoons > 0) {
			...
			while ($currentMoon <= $numMoons) {
				...
				echo "<li>Moon ".$currentMoon."<ul>";
				switch($moonType) {
					case "2":
						echo "<li>Terrestrial Moon</li>";
						break;
					case "4":
						echo "<li>Icy Moon</li>";
						break;
				...
				if ($moonSize[0] <= 7) {
					...
					echo "<li>".$moonBiosphere[2];
					if ($moonBiosphere[0]!=1) {
						echo "<ul><li>".$moonBiosphere[3];
	...
	if ($asteroids == TRUE) {
		...
		foreach ($asteroidBelts as $belt) {
			echo "<ul><li>Asteroid Belt ".$belt."<ul>";
			echo genRoids($modifier);
			echo "</ul></li></ul>";
		}
	}
